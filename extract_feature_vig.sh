#!/bin/bash
#SBATCH --job-name=extract_feature_vig
...
#SBATCH --output=logs/vig_%j.out
#SBATCH --error=logs/vig_%j.err
...
python extract_feature_vig.py \
  --data_h5_dir "/scratch/leuven/373/vsc37341/TCGA-BRCA/downsampled_20x_patch_colornorm" \
  --feat_dir    "/scratch/leuven/373/vsc37341/TCGA-BRCA/downsampled_20x_patch_colornorm_ftvig" \
  --batch_size 256 \
  --num_workers 4 \
  --device cuda
